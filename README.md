...
1. `scripts/gen-og.mjs` renders a **1200×630 PNG share card** per listing —
   branded, carrying the price, title, location and phone number. PNG matters:
   Facebook and WhatsApp will not render an SVG in a preview. The cards are
   brand-led rather than photo-led because listing photos are fetched remotely
   at render time, so there is no local photo to composite and a build should
   not depend on a third-party service.
...
Listing photos are fetched from a random-image endpoint at render time rather
than stored with the listing:

```
https://api.sourcesplash.com/i/random?q=modern%20villa&sig=<slot>
```

`src/lib/photos.ts` owns this. Two details there are load-bearing:

- **Every slot gets a unique `sig`.** Without it, every `<img>` sharing a query
  would resolve to one cached response and a six-photo gallery would show the
  same picture six times.
- **Every image renders through `Photo`** (`src/components/Photo.tsx`), which
  swaps in `public/brand/photo-fallback.svg` on error. The endpoint is
  third-party, so the page must degrade rather than show broken-image icons.

Because the endpoint is random, **a listing's photos change on reload** — they
are illustrative, not the property. Replacing this module with stored photo URLs
per listing is the fix, and nothing outside it needs to change.

## Brand

`brand-src/sk-logo-original.jpg` is the supplied artwork. `scripts/gen-brand.mjs`
keys out its flat grey field, un-premultiplies the edges so no grey halo
survives on other backgrounds, and emits:

- `public/brand/logo-mark.png` — the buildings alone, for the header (the full
  lockup's wordmark is unreadable at 40px)
- `public/brand/logo-full.png` — mark plus wordmark
- `public/favicon.png`

The artwork's navy disappears on a dark background, so the footer and the share
cards put the lockup on a white tile rather than inverting it. Re-run
`npm run gen:brand` after changing the source artwork.

## Known limits (no backend yet)

- Listing photos are random on every reload (see **Imagery**).
...
scripts/
  gen-brand.mjs  logo cut-outs + favicon -> public/brand
